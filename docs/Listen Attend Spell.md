...
- The speller is an [[Attention|attention]]-based RNN decoder that attends to the high level [[Features|features]] and spells out the transcript one character at a time
...
- used samples from the [[Softmax|softmax]] classifier in the decoder as inputs to the next step prediction during training
