...
## Abstract
- [humanoid vision engine](humanoid vision engine.qmd) (HVE) that explicitly and separately computes shape, texture, and color features from images
...
- [Humanoid Vision Engine](Humanoid Vision Engine.qmd)
...
- After we get the segmentation of the image, we use a pre-trained CNN and [Grad-CAM](Grad-CAM.qmd) [47] to find the foreground object among all masks.
...
- For instance, we can imagine a zebra when seeing its stripe (texture). This process is similar but harder than the classical image generation task since the input features [modality](modality.qmd) here dynamic which can be any feature among shape, texture, or color
