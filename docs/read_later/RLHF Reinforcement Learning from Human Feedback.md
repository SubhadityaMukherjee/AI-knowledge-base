...
**Table of contents**  
[RLHF overview](#rlhf_overview)  
[Phase 1. Pretraining for completion](#phase_1_pretraining_for_completion)  
…. [Language model](#language_model)  
…. [Mathematical formulation](#mathematical_formulation_lm)  
…. [Data bottleneck for pretraining](#data_bottleneck_for_pretraining)  
[Phase 2. Supervised finetuning (SFT) for dialogue](#phase_2_sft)  
…. [Why SFT](#why_sft)  
…. [Demonstration data](#demonstration_data)  
…. [Mathematical formulation](#mathematical_formulation_sft)  
[Phase 3. RLHF](#phase_3_rlhf)  
…. [3.1. Reward model (RM)](#3_1_reward_model)  
…….. [Mathematical formulation](#mathematical_formulation_rm)  
…….. [UI to collect comparison data](#ui_to_collect_comparison_data)  
…. [3.2. Finetuning using the reward model](#3_2_finetuning_using_the_reward_model)  
…….. [Mathematical formulation](#mathematical_formulation_rl)  
…. [RLHF and hallucination](#rlhf_and_hallucination)  
...
![3 phases of ChatGPT development](/assets/pics/rlhf/2-shoggoth.jpg)
...
![RedPajama vs. LLaMa data](/assets/pics/rlhf/4-1t-tokens.png)
...
Once the publicly available data is exhausted, the most feasible path for more training data is by relying on proprietary data. I suspect that any company that somehow gets its hand on a massive amount of proprietary data – copyrighted books, translations, contracts, medical records, genome sequences, user data, etc. – will be at a competitive advantage. It’s not surprising that in light of ChatGPT, many companies have changed their data terms to prevent other companies from scraping their data for LLMs – see [Reddit](https://www.redditinc.com/policies/data-api-terms), [StackOverflow](https://policies.stackoverflow.co/teams/enterprise-cloud-business/).
...
![3 phases of ChatGPT development](/assets/pics/rlhf/9-sft-rlhf.png)
...
For each prompt \(x\), the reward function is computed as follows. Note that in the RL setting, we maximize the objective instead of minimizing the objective as done in the previous steps.
...
![RLHF makes hallucination worse](/assets/pics/rlhf/10-hallucination.png)
