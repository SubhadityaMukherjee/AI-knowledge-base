...
- no need for any [Markov_Chain](Markov_Chain.md) or unrolled approximate inference networks during either training or generation of samples
- [Adversarial_Learning](Adversarial_Learning.md)
- ![im](images/Pasted%20image%2020220310210209.png)
- Min Max game $$max_D min_G V(G,D)$$ where $$V(G,D) = \mathbb{E}_{p_{data}(x)}logD(x) + \mathbb{E}_{p_{data}(x)}log(1-D(x))$$
- G : [Gradient_Descent_gradients](Gradient_Descent_gradients.md)
- D : [Gradient_Ascent](Gradient_Ascent.md)
