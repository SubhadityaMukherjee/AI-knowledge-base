...
- The proposed method was tested on various datasets including [CIFAR]-10, [CIFAR](CIFAR]-10, [CIFAR.qmd) 100, SVHN, ImageNet, and COCO, and showed improvement in performance without the use of a proxy task.
...
- RandAugment is able to achieve comparable or better performance compared to other automated augmentation methods, such as [AutoAugment](AutoAugment.qmd), without the need for a separate proxy task.
...
- For each of these networks, we train the model on [CIFAR](CIFAR.qmd)-10 and measure the final accuracy compared to a baseline model trained with default data augmentations (i.e. horizontal flips and pad-and-crop)
...
- Namely, larger networks demand larger data distortions for [regularization](regularization.qmd)
- Conversely, a policy learned on a proxy task (such as [AutoAugment](AutoAugment.qmd)) provides a fixed distortion magnitude (Figure 1b, dashed line) for all architectures that is clearly sub-optimal.
...
- Both parameters are human-interpretable such that larger values of N and M increase [regularization](regularization.qmd) strength
...
- We see that while [geometric transformations](geometric transformations.qmd) individually make the most difference, some of the color transformations lead to a degradation of validation accuracy on average
...
- The 1st -order method improves accuracy by more than 3.0% for both models on reduced [CIFAR](CIFAR.qmd)-10 compared to the baseline of flips and pad-and-crop
...
- In previous work, scaling learned data augmentation to larger dataset and models have been a notable obstacle. For example, [AutoAugment] and Fast [AutoAugment](AutoAugment] and Fast [AutoAugment.qmd) could only be optimized for small models on reduced subsets of data
