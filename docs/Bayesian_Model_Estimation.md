...
- [Bayesian_Prior](Bayesian_Prior.md)
- Now there are two sources of info about the true distribution $p_{X}(\theta)$
	...
	- Since these are independant sources we can combine them by multiplication: $p_{\otimes_{i}}x(D|\theta)h(\theta)$
		...
		- [Bayesian_Posterior](Bayesian_Posterior.md)
		- [Posterior_Mean_estimate](Posterior_Mean_estimate.md)
...
- How to encode or represent [Bayesian_Posterior](Bayesian_Posterior.md) as very high dim
...
- The [Posterior_Mean_estimate](Posterior_Mean_estimate.md) is obtained by integrating $\int_{\mathbb{R}}\mu h(\mu|D)d\mu$
- Since this is different from sample mean -> Prior distribution really does influence the models

### [Protein_Modeling](Protein_Modeling.md)
