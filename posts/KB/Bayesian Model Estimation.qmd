...
# [Bayesian](Bayesian.qmd) Model Estimation
- Unlike [frequentist](Frequentist.qmd), sometimes things like sample mean is not a good metric because it has a high variance. Might give different results with different trials in a real valued distribution
- The task is to estimate $\theta$ from the data
- [Bayesian Prior](Bayesian%20Prior.qmd)
- Now there are two sources of info about the true distribution $p_{X}(\theta)$
	...
	- Since these are independant sources we can combine them by multiplication: $p_{\otimes_{i}}x(D|\theta)h(\theta)$
		...
		- [Bayesian Posterior](Bayesian%20Posterior.qmd)
		- [Posterior Mean estimate](Posterior%20Mean%20estimate.qmd)
...
- How to encode or represent [Bayesian Posterior](Bayesian%20Posterior.qmd) as very high dim
...
- The [Posterior Mean estimate](Posterior%20Mean%20estimate.qmd) is obtained by integrating $\int_{\mathbb{R}}\mu h(\mu|D)d\mu$
- Since this is different from sample mean -> Prior distribution really does influence the models

### [Protein Modeling](Protein%20Modeling.qmd)
