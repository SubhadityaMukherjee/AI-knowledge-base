...
- based on the [GPT3](GPT3.qmd)
- Previous approaches like [BERT](BERT.qmd) and the original [GPT](GPT.qmd) model followed the fine-tuning approach.
- [GPT](GPT.qmd)-2 and [GPT](GPT.qmd)-3 recognized that even while pretraining already provided lots of benefits compared to training from scratch, so-called zero-shot learning - where the model is finetuned and then applied to language tasks, without pretraining - could be the way forward.
...
- DALL-E which offers a simple approach for text-to-image generation based on an [autoregressive](autoregressive.qmd) transformer which models the text and image tokens as a single stream of data
