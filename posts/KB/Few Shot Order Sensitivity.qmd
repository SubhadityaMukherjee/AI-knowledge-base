...
# Few Shot Order [Sensitivity](Sensitivity.qmd)
...
- When primed with only a handful of training samples, very large, pretrained language models such as [GPT](GPT.qmd)-3 have shown competitive results when compared to fully-supervised, fine-tuned, large, pretrained language models
- few-shot prompts suffer from order [sensitivity](Sensitivity.qmd)
...
- identity performant permutations for prompts using [entropy](Entropy.qmd)-based statistics over this set, which yields a 13% relative improvement for [GPT](GPT.qmd)-family models across eleven different established text classification tasks
