...
## Disclaimer
This is an intermediate level article and introduces a significant number of new terms. Attempting to understand this article is not recommended before mastering how a [../../Basic GAN|basic GAN](../../Basic GAN|basic GAN.md) (eg: DCGAN) works.
...
Many networks such as the UNet and GANs have a two sided architecture that involves [../../Downsampling|downsampling](../../Downsampling|downsampling.md) the image until a point and then upsampling from there on. The Encoder is the first half which downsamples the image and condenses the information in a batch of data down into the smallest possible unit. 
