...
First, we create a function that creates blocks of Convolutions, Batch Normalisation, and ReLU with different input and output channels. We give this function the option of having a [../../Dropout|Dropout](../../Dropout|Dropout.qmd) layer at the end or skipping that layer.
...
for i, data in enumerate(dl_eval, 0):
    ...
    print(label)
    if label == torch.FloatTensor([0](0.qmd)):
