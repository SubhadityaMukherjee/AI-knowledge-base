...
# [[Bayesian.md|Bayesian]] Model Estimation
- Unlike [[Frequentist.md|frequentist]], sometimes things like sample mean is not a good metric because it has a high variance. Might give different results with different trials in a real valued distribution
- The task is to estimate $\theta$ from the data
- [[Bayesian Prior.md|Bayesian Prior]]
- Now there are two sources of info about the true distribution $p_{X}(\theta)$
	...
	- Since these are independant sources we can combine them by multiplication: $p_{\otimes_{i}}x(D|\theta)h(\theta)$
		...
		- [[Bayesian Posterior.md|Bayesian Posterior]]
		- [[Posterior Mean estimate.md|Posterior Mean estimate]]
...
- How to encode or represent [[Bayesian Posterior.md|Bayesian Posterior]] as very high dim
	- No closed form representation over weights
	- Represent data with histograms and use [[Monte Carlo]]

## Example
- ![[Pasted image 20220316120743.png|im]]
- ![[Pasted image 20220316120803.png|im]]
- Green : prior , Red: Posterior
- The [[Posterior Mean estimate.md|Posterior Mean estimate]] is obtained by integrating $\int_{\mathbb{R}}\mu h(\mu|D)d\mu$
- Since this is different from sample mean -> Prior distribution really does influence the models

### [[Protein Modeling.md|Protein Modeling]]
