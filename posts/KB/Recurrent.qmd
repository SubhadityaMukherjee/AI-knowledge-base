...
- Activation usually [Sigmoid](Sigmoid.qmd) or [Tanh](Tanh.qmd)
- BPTT
	...
	- #gradients
		...
		- Element wise clipping #tricks
			- [Clip](CLIP.qmd) if bigger than value
		- Norm clipping
			- [Clip](CLIP.qmd) if $$||g|| >v$ set $g = \frac{gv}{||g||}$
			- v can be decided by trial and error
- Training stuff
	- [Softmax](Softmax.qmd) but on every output vector simultaneously
		- If [](Softmax.qmd#T%20is%20the%20Temperature) is lower (eg between 0 and 0.5). It becomes more confident and hence more conservative
