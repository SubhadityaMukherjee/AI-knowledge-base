...
- [Transposed Conv] , [Batch Normalization] and [Relu](Transposed Conv] , [Batch Normalization] and [Relu.qmd)
- Output is 3x64x64
- Output passed through [Tanh](Tanh.qmd) to return it to [-1,1]
- [Batch Normalization] AFTER [Transposed Conv](Batch Normalization] AFTER [Transposed Conv.qmd) is super important as it helps with flow of gradients
...
### Discriminator
- [Strided] [Conv], [Batch Normalization], and [Leaky Relu](Strided] [Conv], [Batch Normalization], and [Leaky Relu.qmd)
- 3x64x64 input
- Binary classification network - outputs prob of real/fake
- Final is a [Sigmoid](Sigmoid.qmd) layer
- For [downsampling], good Practise to use [Strided] rather than [Pooling](downsampling], good Practise to use [Strided] rather than [Pooling.qmd) as it lets the network learn it's own pooling function
...
-   Further the discriminator uses batch norm layers and [Leaky Relu] activations while the generator uses [Relu](Leaky Relu] activations while the generator uses [Relu.qmd) activations
-   The input is a latent vector drawn from a standard [normal distribution](normal distribution.qmd) and the output is a $3 \times 32 \times 32$ RGB image
-   In this implementation, I also added in [Label Smoothing](Label Smoothing.qmd)

## Loss functions
### Discriminator [loss](loss.qmd)
