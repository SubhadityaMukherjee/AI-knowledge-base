...
As can be seen in the test error, that helped quite a bit! Data augmentation is a fairly simple and very standard concept used to fight overfitting, but I didn’t see it mentioned in the 1989 paper, perhaps it was a more recent innovation (?). Since we are still overfitting a bit, I reached for another modern tool in the toolbox, [Dropout](https://pytorch.org/docs/stable/generated/torch.nn.Dropout.html). I added a weak [[../Dropout|dropout]] of 0.25 just before the layer with the largest number of parameters (H3). Because [[../Dropout|dropout]] sets activations to zero, it doesn’t make as much sense to use it with tanh that has an active range of [-1,1], so I swapped all non-linearities to the much simpler [ReLU](https://pytorch.org/docs/stable/generated/torch.nn.ReLU.html) activation function as well. Because [[../Dropout|dropout]] introduces even more noise during training, we also have to train longer, bumping up to 80 passes, but giving:
