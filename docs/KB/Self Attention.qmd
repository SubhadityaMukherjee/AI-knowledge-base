...
# Self [Attention](Attention.qmd)
- [paper](https://arxiv.org/abs/1706.03762v5)
- Basically [Scaled Dot Product Attention](Scaled%20Dot%20Product%20Attention.qmd)
...
- Weighted average over all input vectors $$y_{i}= \Sigma_{j}w_{ij}x_{j}$$
	...
	- $w_{ij}$ is derived $w^{'}_{ij}=x_{i}^{T}x_{j}$
		- Any value between -inf to +inf so [Softmax](Softmax.qmd) is applied
...
- The process
	- Assign every word t in the vocabular an [Embedding](Embedding.qmd)
	- Feeding this into a self [attention](Attention.qmd) layer we get another seq of vectors $y_{the}$ , $y_{cat}$ etc
	- each of the $y_{something}$ is a weighted sum over all the [embedding](Embedding.qmd) vectors in the first seq weighted by their normalized dot product with $v_{something}$
	- the dot product shows how related the vectors are in the sequence
		- weights determined by them
		- Self-[Attention](Attention.qmd) layer may give more weights to those input vectors that are more similar to each other when generating the output vectors
- Properties
