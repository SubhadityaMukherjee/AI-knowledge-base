...
- Rescorla & Wagner (1972): animals and humans also learn associations by paying [[Attention|attention]] to what is not associated.
- $$\Delta V = \alpha \beta_{1}\beta_{2}(\lambda = \Sigma V)$$
- ▶ V = association strength ▶ ∆ V : Change in association strength ▶ λ = maximum values of the unconditional stimulus ▶ Set to 1: when US is present (food) ▶ Set to 0: when not present ▶ α = learning rate ▶ β = varies the effects of negative or positive evidence ▶ ΣV = sum of associated strengths for all cues/[[Features|features]]/conditions stimuli
