...
- Given the importance of local context, the sliding window attention pattern employs a fixed-size window attention surrounding each token
    - multiple stacked layers of such windowed attention results in a large [receptive field](Receptive%20field.md), where top layers have access to all input locations and have the capacity to build representations that incorporate information across the entire input
