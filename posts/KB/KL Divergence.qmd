...
# KL Divergence
- Classification
- [Entropy](Entropy.qmd) + [Cross Entropy](Cross%20Entropy.qmd)
- Distribution Based metric
- Measures difference between two [PDF](PDF.qmd)
- We first define xlogx for a weird edge case $$x \cdot \log\left( x \right)$$

Then [entropy](Entropy.qmd) $$\mathrm{sum}\left( \mathrm{xlogx}\left( y \right) \right) \cdot \mathrm{//}\left( 1, \mathrm{size}\left( y, 2 \right) \right)$$
