...
- General Ideas
    ...
    - **Strong usage of small convolutional kernels**(e.g. 3×3): Throughout the network the authors pefer smaller convolutional kernel size over larger ones, as this enables the same [receptive field](Receptive%20field.md) with less parameters (e.g. a single 5×5convolution [∼25 params ](Receptive%20field.md) as 2 consecutive 3×3convolutions [∼18 params ], but the later has less parameters)
