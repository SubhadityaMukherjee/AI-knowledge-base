...
- analyze the effect of training iterations, compare networks trained with different initializations, examine the impact of network depth and width, and measure the effect of [dropout](dropout.qmd) and batch normalization on the interpretability of deep visual representations
...
- evaluates every individual convolutional unit in a CNN as a solution to a binary segmentation task to every visual concept in [Broden](Broden.qmd)
- applied to any CNN using a forward pass without the need for training or backpropagation.
- For every input image x in the [Broden](Broden.qmd) dataset, the activation map $A_{k}(x)$ of every internal convolutional unit k is collected.
...
- For the network without [dropout](dropout.qmd), more texture detectors emerge but fewer object detectors
