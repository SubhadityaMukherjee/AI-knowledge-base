...
- Step analysis:
- These are conclusions that are produced by applying knowledge components. For instance, the first two items above correspond to distinct learning events, wherein the student has applied the same [Knowledge_Component](Knowledge_Component.md),
...
- To find out which learning events underlie the student's step, AutoTutor measures the semantic similarity between the text of the [Learning_Event](Learning_Event.md) and the text of the step. It uses a measure called [Latent_Semantic_Analysis](Latent_Semantic_Analysis.md)
