...
# Teacher Forcing
- [from](https://publish.obsidian.qmd/fabian-groeger/Machine+Learning+%26+Deep+Learning/Deep+Learning/Architectures/RNN/Teacher+Forcing)
...
- common technique to train [Basic RNN Architectures](Basic%20RNN%20Architectures.qmd) or [Transformer](Transformer.qmd)
    - used in [imageCaptioning](imageCaptioning.qmd) , Machine Translation
...
- [loss](loss.qmd) does not need to be updated at each timestep, only needs to have a list with the true predictions of the model from which then the [loss](loss.qmd) is calculated
- pros
