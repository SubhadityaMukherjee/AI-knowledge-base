...
A Support Vector Machine (SVM) is a supervised classification and regression algorithm that uses the concept of **hyperplanes**. These hyperplanes can be understood as **multi-dimensional linear [../../Decision Boundaries|decision boundaries](../../Decision Boundaries|decision boundaries.md)** that separate groups of unequal data points. 
...
Decision [../../Trees|trees](../../Trees|trees.md) are tree-based decision models that use an internal structure of a **root node** followed by successive child **leaf nodes**. The leaf nodes are a placeholder for the classification label, and the branches show the outcomes of the decision. The paths from the tree's root to the leaves represent the classifier rules. Each tree and sub-tree models a single decision and enumerates all the possible decisions to choose the best one.
A Decision tree can be optimal if it represents most of the data with the least number of levels. Decision [../../Trees|trees] are helpful for classification but can be extended for Regression using different algorithms. These [../../Trees|trees](../../Trees|trees] are helpful for classification but can be extended for Regression using different algorithms. These [../../Trees|trees.md) are computationally efficient, and many tree-based optimizations have been created over the years to make them perform even faster.
...
Random Forest models use a forest of Decision [../../Trees|Trees] to make better decisions by combining each tree's decisions. The most popular decision across the [../../Trees|trees](../../Trees|Trees] to make better decisions by combining each tree's decisions. The most popular decision across the [../../Trees|trees.md) for a task is the best after the aggregation. This technique of aggregating multiple results from similar processes is called **Ensembling**. 
