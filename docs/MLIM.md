...
- determines the masking probability and applies masking to both word and image [[Embedding|embedding]]
- based on BERT predict the masked words from available words and image regions
- follow BERT for this task: two-layer MLP MLM head outputting [[Logits|logits]] over the vocabulary
- MLM loss is negative log-likelihood for masked word
- RECON loss is an an average of pixel-wise sum of squared errors (SSE)
- Both image and word masking is realized by replacing an [[Embedding|embedding]] with the [[Embedding|embedding]] of `[MASK]`
- transformer [[Layers|layers]] recognize `[MASK]`
- ’s [[Embedding|embedding]] as a special [[Embedding|embedding]] that needs to be “filled in”, independent of the modality, by attending to other vectors in the layer inputs
...
- However, RECON instead of [[ITM Loss|ITM loss]] offers better PR AUC
- Similarly, using the [[ITM Loss|ITM loss]] together with MLM and RECON does not change the performance
