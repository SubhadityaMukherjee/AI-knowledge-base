...
- performance bug detection approaches have emerged to help developers identify performance [issues](Issues.qmd)
- Building rule-based analyzers is a non-trivial task, as it requires achieving the right balance between precision and [recall](recall.qmd)
- Once developed, maintaining these rules can also be costly
- large [transformer](Transformer.qmd) model to suggest changes at application source code level to improve its performance
