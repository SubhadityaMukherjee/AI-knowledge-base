...
### Generator Loss
Since the Generator's objective is to create better fake images gradually, it needs to minimize the difference between the predicted image and the target. The model uses [[../../One hot.md|one hot.md|../../One hot|one hot]] encoded label in this architecture to decide which features to care for. The loss function thus is the following.
