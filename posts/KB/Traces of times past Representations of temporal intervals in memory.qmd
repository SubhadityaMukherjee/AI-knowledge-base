...
- These findings support a model in which the representation of a time interval is not a single [memory trace](memory trace.qmd), but a pool of experiences in which recency and match to the current request determine the impact of single experiences.
- The main specific choice we made in this model was to treat every experience with each of the intervals as a separate [memory trace](memory trace.qmd)
