...
- The converted vectors can be compared by using distance metrics such as the [../../Cosine Distance|cosine distance](../../Cosine Distance|cosine distance.md) function $cos(\theta) = \frac{A\cdot B}{||A||\cdot||B||}$ ( where A and B are vectors). These metrics make it easy to find related words like the one we want to achieve.
