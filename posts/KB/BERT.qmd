...
- Uses [Token Embedding](Token%20Embedding.qmd)
- [Self Supervised](Self%20Supervised.qmd)
- Masked [language](language.qmd) modeling, next sentence prediction
- ![im](images/Pasted%20image%2020220307183916.png)
- [CLS] : start of classification task, [SEP] between sentences, [MASK] : masked token
- [christianversloot](https://github.com/christianversloot/machine-learning-articles/blob/main/intuitive-introduction-to-bert.qmd) #[Roam-Highlights](Roam-Highlights)
    - BERT base $\text{BERT}_\text{BASE}$, which has 12 Encoder Segments stacked on top of each other, has 768-dimensional intermediate state, and utilizes 12 [attention](Attention.qmd) heads (with hence 768/12 = 64-dimensional [attention](Attention.qmd) heads).
    - BERT large ($\text{BERT}_\text{LARGE}$), which has 24 Encoder Segments, 1024-dimensional intermediate state, and 16 [attention](Attention.qmd) heads (64-dimensional [attention](Attention.qmd) heads again).
    ...
    - [Masked Language Modeling](Masked Language Modeling.qmd)
    ...
    - [BooksCorpus](BooksCorpus.qmd)
    - [English Wikipedia](English%20Wikipedia.qmd)
