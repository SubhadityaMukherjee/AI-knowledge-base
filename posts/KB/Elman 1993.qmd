...
- examine hidden unit activation pattern for each word measure distance between each pattern and every other pattern ([Euclidean distance](Euclidean%20Distance.qmd))
- Use this to create a hierarchical cluster.
- Network learned semantic classes
- If the input to a simulation is preselected to avoid problems, one has instantiated an expert [filtering](Filtering.qmd) system.
...
- Yes, the input was oversimplified, but it's not clear that adding these additional [features](Features.qmd) will make the model perform worse
- language is very redundant, so certain simplifications actually remove helpful [features](Features.qmd)
- Categories can 'emerge' via statistical regularities
- [Basic RNN Architectures](Basic%20RNN%20Architectures.qmd) can find these
