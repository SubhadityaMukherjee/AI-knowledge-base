...
- idea is to [[CLIP|clip]] the gradients during Backpropagation to a certain threshold (limit the value)
- most often used in RNN or GAN, where Batch Normalisation is tricky to use
- methods
    - [[CLIP|clip]] by norm
        - [[CLIP|clip]] the whole gradient if its L2 norm is greater than the threshold
        - remains the orientation
    - [[CLIP|clip]] by value
        - [[CLIP|clip]] the gradient by a fixed value
