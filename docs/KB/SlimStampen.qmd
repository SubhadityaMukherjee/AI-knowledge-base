...
- The model represents every encountered item by a unique memory chunk, based on the [ACT-R](ACT-R.qmd) theory of declarative memory (Anderson, 2007).
...
- A correct but slower-than-expected response signals that the [memory trace](memory trace.qmd) has decayed further than assumed, meaning that the item’s true rate of forgetting is higher than the current estimate.
- That is, when a learner arrives at the right answer but takes longer than anticipated, they likely struggled to [recall](recall.qmd) the information
- Conversely, an incorrect or missing response suggests that the activation of the
- item’s [memory trace] actually dropped below the retrieval threshold, which means that the true rate of forgetting should be higher because this item’s activation was expected to be above the threshold (which was fixed at [ACT-R](memory trace] actually dropped below the retrieval threshold, which means that the true rate of forgetting should be higher because this item’s activation was expected to be above the threshold (which was fixed at [ACT-R.qmd)’s default value).
- An unexpectedly fast correct response, on the other hand, indicates a stronger- than-expected [memory trace](memory trace.qmd) and implies that the estimated rate of forgetting should be adjusted downward.
...
- The advantage of lasso regression is that the [shrinkage](Shrinkage.qmd) term handles multicollinearity between the predictors by shrinking their coefficients
- The [shrinkage](Shrinkage.qmd) is achieved by imposing a cost function on the magnitude of the
- coefficients themselves: the best fit is achieved by the model that minimizes the OLS with the smallest coefficients. In fact, coefficients are shrunk entirely if they do not explain sufficient variance to justify inclusion in the model. In lasso regression, predictors must be normalized to ensure that the [shrinkage](Shrinkage.qmd) term affects all predictors equally. A convenient consequence of normalized predictors is that their post-[shrinkage](Shrinkage.qmd)
