...
- scaled variant of the [Elu](Elu.qmd) function
- does internal normalization ("self-normalizing")
    ...
    - to work:
        ...
        - SELU as activation 
        - custom [Initialization](Initialization.qmd)
            - zero mean
            - [standard deviation](Standard%20Deviation.qmd): $\sqrt{\frac{1}{ \#inputs}}$
        - if all [layers](Layers.qmd) are dense (in paper), but other research showed that it also works for CNNs
- has two fixed parameters α and λ
    - not hyperparameters nor learnt parameters
    - derived from the inputs (μ=0, std=1)
    - α≈1.6732, λ≈1.0507
- Pros:
    - no [Vanishingexploding gradients](Vanishingexploding%20gradients.qmd)
    - cannot die as [Relu](Relu.qmd)
