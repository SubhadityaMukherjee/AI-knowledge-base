...
- idea is to [clip](CLIP.qmd) the gradients during Backpropagation to a certain threshold (limit the value)
- most often used in RNN or GAN, where Batch Normalisation is tricky to use
- methods
    - [clip](CLIP.qmd) by norm
        - [clip](CLIP.qmd) the whole gradient if its L2 norm is greater than the threshold
        - remains the orientation
    - [clip](CLIP.qmd) by value
        - [clip](CLIP.qmd) the gradient by a fixed value
...
- [Adaptive Gradient Clipping](Adaptive%20Gradient%20Clipping.qmd)
