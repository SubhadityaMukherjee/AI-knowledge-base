...
- hey are in the same basins of the loss landscape models trained from random [initialization](Initialization.qmd) do not live in the same basin, make different mistakes, have different features and are farther away in 2 distance in the parameter space
...
- T: trained, P: Pre-trained, RI: random [initialization](Initialization.qmd). Therefore we use the following abbreviations for the four models throughout the paper: RI (random [initialization](Initialization.qmd)), P (pre-trained model), RI-T (model trained on target domain from random [initialization](Initialization.qmd)), P-T (model trained/fine-tuned on target domain starting from pre-trained weights) 
- IMAGENET 
- CHEXPERT
- DOMAINNET
...
- These experiments show that the [initialization](Initialization.qmd) point, whether pre-trained or random, drastically impacts feature similarity, and although both networks are showing high accuracy, they are not that similar in the feature space. two P-T are reusing the same features
...
## Conclusion
- We investigated the role of feature reuse through shuffling the blocks of input and showed that when trained from pre-trained weights [initialization](Initialization.qmd), the network stays in the same basin of the solution, features are similar and models are close in the L2 distance in parameter space confirmed that lower layers are in charge of more general features
- one can use top singular values and directions for each module for [initialization](Initialization.qmd) and investigate if this suffices for good transfer, or ensure [initialization](Initialization.qmd) at the same basin but adding randomization to enhance diversity and improve generalization
